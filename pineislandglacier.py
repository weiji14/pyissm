...
z_attribute = "slipperiness"
# z_attribute = "rheology"
z_attribute = "basal_drag"
# z_attribute = "pressure"
...
if z_attribute == "rheology":
    ...
    if z_attribute in ("slipperiness", "pressure", "basal_drag"):
        df = dfFS.query(expr="isbasal == True")
...
y = df.y
z = df[z_attribute]
data: pd.DataFrame = df[["x", "y", z_attribute]]
xmin, xmax, ymin, ymax, zmin, zmax = pygmt.info(table=data, per_column=True)
...
fig.basemap(
    ...
    frame=["af", f'WSne+t"{bedname} {z_attribute} {symbol}"'],
)
fig.plot(x=x, y=y, style="c0.01c")
fig.contour(
    x=x, y=y, z=z, I=True, levels=True, label_placement="l-1750000/-350000/-1400000/0"
)
...
fig.text(
    position="TR",
    text=rf"@!\257{symbol}={grid.mean().item():.4e}",
    justify="TR",
    offset="-0.2c/-0.2c",
)
...
fig.colorbar(
    position="JRM", S=True, frame="1000a" if z_attribute == "basal_drag" else None
)
